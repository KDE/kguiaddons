...
static constexpr QLatin1StringView b83Characters{"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz#$%*+,-.:;=?@[]^_{|}~"};
...
KBlurHash::Components KBlurHash::unpackComponents(const int packedComponents)
{
    return {packedComponents % 9 + 1, packedComponents / 9 + 1};
}

float KBlurHash::decodeMaxAC(const int value)
{
    return static_cast<float>(value + 1) / 166.f;
}

QColor KBlurHash::decodeAverageColor(const int encodedValue)
{
...
float KBlurHash::signPow(const float value, const float exp)
{
    return std::copysign(std::pow(std::abs(value), exp), value);
}

QColor KBlurHash::decodeAC(const int value, const float maxAC)
{
...
QList<float> KBlurHash::calculateWeights(const qsizetype dimension, const qsizetype components)
{
