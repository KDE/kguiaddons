...
class WlrDataControlDeviceManager : public QWaylandClientExtensionTemplate<WlrDataControlDeviceManager>, public QtWayland::zwlr_data_control_manager_v1
{
    Q_OBJECT
public:
    WlrDataControlDeviceManager()
        : QWaylandClientExtensionTemplate<WlrDataControlDeviceManager>(2)
    {
    }
...
class WlrDataControlOffer : public QMimeData, public QtWayland::zwlr_data_control_offer_v1
{
    Q_OBJECT
public:
    WlrDataControlOffer(struct ::zwlr_data_control_offer_v1 *id)
...
QVariant WlrDataControlOffer::retrieveData(const QString &mimeType, QMetaType type) const
{
    Q_UNUSED(type);
    ...
    auto t = const_cast<WlrDataControlOffer *>(this);
...
bool WlrDataControlOffer::readData(int fd, QByteArray &data, const QString &mimeType)
{
    pollfd pfds[1];
    pfds[0].fd = fd;
    ...
    while (true) {
        ...
        if (ready < 0) {
            if (errno != EINTR) {
                qWarning("WlrDataControlOffer: poll() failed for mimeType %s: %s", qPrintable(mimeType), strerror(errno));
                return false;
            }
        } else if (ready == 0) {
            qWarning("WlrDataControlOffer: timeout reading from pipe for mimeType %s", qPrintable(mimeType));
            return false;
        } else {
            char buf[4096];
            ...
            if (n < 0) {
                qWarning("WlrDataControlOffer: read() failed for mimeType %s: %s", qPrintable(mimeType), strerror(errno));
                return false;
            } else if (n == 0) {
                return true;
            } else if (n > 0) {
...
class WlrDataControlSource : public QObject, public QtWayland::zwlr_data_control_source_v1
{
    Q_OBJECT
public:
    WlrDataControlSource(struct ::zwlr_data_control_source_v1 *id, QMimeData *mimeData);
    WlrDataControlSource() = default;
    ~WlrDataControlSource()
    {
        destroy();
    }
...
WlrDataControlSource::WlrDataControlSource(struct ::zwlr_data_control_source_v1 *id, QMimeData *mimeData)
...
void WlrDataControlSource::zwlr_data_control_source_v1_cancelled()
{
    Q_EMIT cancelled();
}

class WlrDataControlDevice : public QObject, public QtWayland::zwlr_data_control_device_v1
{
    Q_OBJECT
public:
    WlrDataControlDevice(struct ::zwlr_data_control_device_v1 *id)
        : QtWayland::zwlr_data_control_device_v1(id)
    {
    }

    ~WlrDataControlDevice()
    {
        destroy();
    }

    void setSelection(std::unique_ptr<WlrDataControlSource> selection);
    ...
    void setPrimarySelection(std::unique_ptr<WlrDataControlSource> selection);
...
protected:
    ...
    void zwlr_data_control_device_v1_selection(struct ::zwlr_data_control_offer_v1 *id) override
    {
        if (!id) {
            ...
            auto offer = dynamic_cast<WlrDataControlOffer *>(derivated); // dynamic because of the dual inheritance
...
private:
    std::unique_ptr<WlrDataControlSource> m_selection; // selection set locally
    std::unique_ptr<WlrDataControlOffer> m_receivedSelection; // latest selection set from externally to here

    std::unique_ptr<WlrDataControlSource> m_primarySelection; // selection set locally
    std::unique_ptr<WlrDataControlOffer> m_receivedPrimarySelection; // latest selection set from externally to here
    friend WlrWaylandClipboard;
};
...
class WlrKeyboardFocusWatcher : public QWaylandClientExtensionTemplate<WlrKeyboardFocusWatcher>, public QtWayland::wl_seat
{
    Q_OBJECT
public:
    WlrKeyboardFocusWatcher()
    ...
    ~WlrKeyboardFocusWatcher() override
    {
        if (isActive()) {
            release();
        }
    ...
    {
        const bool hasWlrKeyboard = capabilities & capability_keyboard;
        if (hasWlrKeyboard && !m_keyboard) {
            m_keyboard = std::make_unique<WlrKeyboard>(get_keyboard(), *this);
...
private:
    friend WlrKeyboard;
    bool m_focus = false;
    std::unique_ptr<WlrKeyboard> m_keyboard;
};

class WlrKeyboard : public QtWayland::wl_keyboard
{
public:
    WlrKeyboard(::wl_keyboard *keyboard, WlrKeyboardFocusWatcher &seat)
...
private:
    ...
    WlrKeyboardFocusWatcher &m_seat;
};

WlrWaylandClipboard::WlrWaylandClipboard(QObject *parent)
    : KSystemClipboard(parent)
    , m_keyboardFocusWatcher(new WlrKeyboardFocusWatcher)
    , m_manager(new WlrDataControlDeviceManager)
{
    connect(m_manager.get(), &WlrDataControlDeviceManager::activeChanged, this, [this]() {
        if (m_manager->isActive()) {
            ...
            m_device.reset(new WlrDataControlDevice(m_manager->get_data_device(seat)));

            connect(m_device.get(), &WlrDataControlDevice::receivedSelectionChanged, this, [this]() {
            ...
            connect(m_device.get(), &WlrDataControlDevice::selectionChanged, this, [this]() {
                Q_EMIT changed(QClipboard::Clipboard);
            });

            connect(m_device.get(), &WlrDataControlDevice::receivedPrimarySelectionChanged, this, [this]() {
                // When our source is still valid, so the offer is for setting it or we emit changed when it is cancelled
                if (!m_device->primarySelection()) {
                    Q_EMIT changed(QClipboard::Selection);
                }
            });
            connect(m_device.get(), &WlrDataControlDevice::primarySelectionChanged, this, [this]() {
                Q_EMIT changed(QClipboard::Selection);
            });
...
WlrWaylandClipboard::~WlrWaylandClipboard() = default;

WlrWaylandClipboard *WlrWaylandClipboard::create(QObject *parent)
{
    auto clipboard = new WlrWaylandClipboard(parent);
...
bool WlrWaylandClipboard::isValid()
{
    return m_manager && m_manager->isInitialized();
}

void WlrWaylandClipboard::setMimeData(QMimeData *mime, QClipboard::Mode mode)
{
    if (!m_device) {
        return;
    }
    ...
    connect(m_keyboardFocusWatcher.get(), &WlrKeyboardFocusWatcher::keyboardEntered, this, &WlrWaylandClipboard::gainedFocus, Qt::UniqueConnection);
    auto source = std::make_unique<WlrDataControlSource>(m_manager->create_data_source(), mime);
...
void WlrWaylandClipboard::gainedFocus()
{
    disconnect(m_keyboardFocusWatcher.get(), &WlrKeyboardFocusWatcher::keyboardEntered, this, nullptr);
...
void WlrWaylandClipboard::clear(QClipboard::Mode mode)
{
    if (!m_device) {
        return;
    }
...
const QMimeData *WlrWaylandClipboard::mimeData(QClipboard::Mode mode) const
{
    if (!m_device) {
        return nullptr;
    }
