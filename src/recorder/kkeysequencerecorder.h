...
class KGUIADDONS_EXPORT KKeySequenceRecorder : public QObject
{
     ...
     * must contain only modifier keys (KKeySequenceRecorder::Modifier) or modifiers keys and a
     * normal key (KKeySequenceRecorder::ModifierAndKey).
     *
     * The patterns property can contain one or more recording patterns. For example, if the recorder
     * accepts both normal and modifier only shortcuts, e.g. KKeySequenceRecorder::Modifier | KKeySequenceRecorder::ModifierAndKey.
     *
     * By default this is KKeySequenceRecorder::ModifierAndKey.
     *
     * \since 6.12
     */
...
public:
     ...
     * \value ModifierAndKey
     *        The recorded shortcut must contain one or more modifier keys followed by a regular key,
     *        e.g. Meta+A.
     *
     * \note with KKeySequenceRecorder::ModifierAndKey pattern, special keys like "Insert" without
     * any pressed modifier will be captured too. Please do not rely on this behavior and instead use
     * KKeySequenceRecorder::Key | KKeySequenceRecorder::ModifierAndKey explicitly. The future
     * versions of KKeySequenceRecorder are expected not to record special keys in the KKeySequenceRecorder::ModifierAndKey mode.
     */
    enum Pattern {
        Modifier = 0x1,
        Key = 0x2,
