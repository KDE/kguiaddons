 ...
 *
 * \sa https://en.wikipedia.org/wiki/Hue
 * \since 5.68
 */
 ...
 * \endlist
 *
 * \a amount factor by which to adjust the luma component of the color
 *
 * \a chromaInverseGain (optional) factor by which to adjust the chroma
 ...
 *
 * \a amount factor by which to adjust the luma component of the color
 * \a chromaGain (optional) factor by which to adjust the chroma
 ...
 *
 * \a lumaAmount amount by which to adjust the luma component of the
 * color; 0.0 results in no change, -1.0 turns anything black, 1.0 turns
 * anything white
 *
 * \a chromaAmount (optional) amount by which to adjust the chroma
 ...
 * foreground) from colors in a different class. Therefore when \a amount
 * is low, the luma of \a base is mostly preserved, while the hue and
 * chroma of \a color is mostly inherited.
 *
 * \a base color to be tinted
 *
 * \a color color with which to tint
 *
 * \a amount how strongly to tint the base; 0.0 gives @p base,
 * 1.0 gives @p color
...
/*!
 * Blend two colors into
 * a new color by linear combination.
 * \code
 ...
 * \a  c2 second color.
 *
 * \a bias weight to be used for the mix. \a bias <= 0 gives \a c1,
 *
 * \a bias >= 1 gives \a c2. \a bias == 0.5 gives a 50% blend of \a c1
 * and \a c2.
 *
 ...
 * \endcode
 *
 * \a base the base color (alpha channel is ignored).
 *
 * \a paint the color to be overlaid onto the base color.
 *
 * \a comp the CompositionMode used to do the blending.
 */
